...
export class ReadingBodyImageHost {
  ...
  private configureDisplayCache(cacheDir: string): void {
    ...
    // Versions before the dedicated display directory wrote their transient
    // files directly under cacheDir. They are unreachable after an upgrade,
    // so reclaim only the two exact legacy prefixes before creating v1.
    for (const name of fs.listFileSync(cacheDir)) {
      if (name.startsWith(LEGACY_DISPLAY_FILE_PREFIX) ||
        name.startsWith(LEGACY_DISPLAY_TEMP_PREFIX)) {
