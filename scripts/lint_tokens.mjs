...
// Auto-generated contract sources hold literal Figma values as data (theme
// swatches, screen-graph node bindings, etc.). They are not visual component
// code and are regenerated by tools/codegen — exclude them from the raw-token
// lint so component code remains the only place that needs policing.
const EXCLUDE_DIRS = ['contract/generated', 'contract/reader_ui'];
// This file is the narrowly scoped, revision-null Figma source boundary for
// current reading-chain masters. It is the one hand-authored token file allowed
// to contain literal Figma values; component code must consume its names.
const EXCLUDE_FILES = new Set(['ui/tokens/FigmaReadingVisualTokens.ets']);
...
function walk(dir) {
  for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (e.isDirectory()) {
      ...
      const relativePath = path.relative(ETS, p);
      if (EXCLUDE_FILES.has(relativePath)) continue;
      ...
      lines.forEach((line, i) => {
        ...
        if (HEX.test(line)) offenders.push(`${relativePath}:${i + 1} raw hex: ${line.trim()}`);
        if (PX.test(line)) offenders.push(`${relativePath}:${i + 1} raw px: ${line.trim()}`);
      });
    }
  }
}
