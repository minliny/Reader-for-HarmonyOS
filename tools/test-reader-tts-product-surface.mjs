...
const manager = await readFile(new URL('ReaderTtsConfigOverlay.ets', readingDir), 'utf8');
...
assert.match(ttsBinding, /availableWidth: this\.contentMotionWidth,/);
assert.match(ttsBinding, /availableHeight: this\.contentMotionHeight,/);
assert.match(controls, /this\.contentMotionProgress = f\.progress/);
assert.match(controls, /this\.contentMotionWidth = f\.content\.width/);
assert.match(controls, /this\.contentMotionHeight = f\.content\.height/);
assert.match(ttsBinding, /interactionEnabled: this\.secondaryModuleInputEnabled\('tts'\)/);
assert.match(ttsBinding, /dismissTemporaryRevision: this\.dismissTemporaryRevision/);
assert.match(ttsBinding, /onTemporaryLayerChange:[\s\S]*?this\.onTemporaryLayerChange\(active\)/);
...
assert.match(manager, /if \(this\.current\(generation\)\) this\.errorText = error\.message/,
  'the active form retains failures inline and ignores callbacks after unmount');
