...
const names=new Set();
// Dynamic Reader helpers return complete bare asset names and add app.media at
// the $r call. Admit only complete string literals backed by an existing SVG,
// including those names; do not infer arbitrary runtime string construction.
// test-theme-dynamic-icons enumerates every current computed $r consumer.
function collect(dir){for(const ent of fs.readdirSync(dir,{withFileTypes:true})){const f=path.join(dir,ent.name);if(ent.isDirectory())collect(f);else if(f.endsWith('.ets'))for(const m of fs.readFileSync(f,'utf8').matchAll(/(['"])(?:app\.media\.)?([A-Za-z][A-Za-z0-9_]*)\1/g))if(!m[2].endsWith('_theme_night')&&fs.existsSync(path.join(media,m[2]+'.svg')))names.add(m[2]);}}
collect(path.join(root,'entry/src/main/ets'));
// Read-only discovery receipt, also checked against the live dynamic Builders.
