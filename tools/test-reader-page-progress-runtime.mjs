...
for(const outcome of ['written-lost-reply','origin','unavailable','foreign']){
 ...
 const owner=Object.assign(new Owner(),{bookId:'b',cancelPageTurnSettlementDeadline(){},lastCommittedProgress:{chapterIndex:1,chapterOffset:10,updatedAt:1},isPreparedPageTurnCommitCurrent:()=>true,isMountedToken:()=>true,coreLayout:()=>({}),errorMessage:e=>e.message,finishPreparedPageTurnSettlement(){finished++;},getUIContext:()=>({showAlertDialog:d=>{dialog=d;}}),activeGateway:()=>({runProgressCommitSerial:fn=>fn(),resolveAndUpdateProgress:async()=>{writes++;throw Error('reply lost');},loadProgress:async()=>{reads++;if(outcome==='unavailable')throw Error('offline');return {kind:'restored',progress:{bookId:'b',chapterIndex:outcome==='written-lost-reply'?2:1,chapterOffset:outcome==='written-lost-reply'?40:outcome==='foreign'?77:10,chapterProgress:.4,updatedAt:1}};}})});
