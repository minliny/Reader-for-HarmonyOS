...
contract('manual, gesture, and auto page turns converge before choosing slide or direct mode', () => {
  ...
  assert.match(manual, /return this\.performPageTurn\(direction\);/,
...
contract('settings and Appearance share one ReaderSettings page-turn truth', () => {
  ...
  assert.match(settingsState,
    /style === 'cover'[\s\S]*style === 'slide'[\s\S]*style === 'simulation'[\s\S]*style === 'scroll'[\s\S]*style === 'none'/);

  assert.match(localReading, /const snapshot = setReaderPageTurnStyle\(this\.readerSettingsSnapshot, style\);/);
  assert.match(localReading, /pageTurnStyle: readerPageTurnStyle\(this\.readerSettingsSnapshot\)/,
    'the stage must render the same persisted setting exposed by control pages');
  assert.match(localReading,
  ...
  assert.match(appearancePanel, /return \['覆盖', '平移', '仿真', '滚动', '无动画'\];/);
  assert.match(appearancePanel,
    /const next: ReaderPageTurnStyle = option === '覆盖' \? 'cover'[\s\S]*this\.onPageTurnStyleChange\(next\);/);
  assert.match(appearancePanel, /if \(this\.pageTurnStyle === 'simulation'\)/);
});
...
contract('page-turn generations invalidate stale preparation and settlement owners', () => {
  ...
  assert.match(methodSection(localReading, 'changeReaderPageTurnStyle'), /this\.invalidatePageTurnRuntime\(/,
    'changing animation mode must revoke old stage/cache ownership');
...
contract('a live resize updates presentation geometry without invalidating an in-flight commit', () => {
  ...
  assert.match(admitViewport,
    /if \(this\.pageTurnSettlementActive\)[\s\S]*this\.pageTurnPendingViewportWidth = width;[\s\S]*this\.admitPageTurnPresentationWidth\(width\);[\s\S]*return;/,
    'a settlement must defer logical pagination width while immediately adapting the visible stage');
  assert.ok(admitViewport.indexOf('if (this.pageTurnSettlementActive)') <
  ...
  assert.doesNotMatch(admitViewport.match(/if \(this\.pageTurnSettlementActive\)[\s\S]*?\n    \}/)?.[0] ?? '',
  ...
  assert.match(admitPresentation,
    ...
    'the old page must move a complete new live width after a resize');
  ...
  assert.ok(finish.indexOf('this.pageTurnSettlementLayout = undefined;') <
    finish.indexOf('this.resumeDeferredPageTurnWork();'),
  'new viewport and metrics facts may be admitted only after the transaction join releases its layout snapshot');
  assert.ok(metricsReflow.indexOf('if (this.pageTurnSettlementActive)') <
    metricsReflow.indexOf('const nextLayoutKey = this.windowMetricsLayoutKey();'),
  'a metrics-only change must be deferred before the frozen layout can compare equal to the previous key');
});
