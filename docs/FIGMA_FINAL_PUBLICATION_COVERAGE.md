...
| **Book Detail（local）** | `23 · Pages · Final` `943:651`(Phone) `943:855`(Tablet) | Phone/Tablet | CORE | `bookshelf.get`、`local_book.toc`、`reading.progress.get` | 书架→详情；返回=书架 | ✅ | ✅ | ✅（Phone） | ✅（Phone）/⊘(Tablet) | NoCover：书架卡已按 Figma `3612:1796`（防崩溃+视觉）；Hero 无 NoCover 变体（`N-COVER-HERO-01`）；Songti `B-DETAIL-01` 桥接 | `LocalBookDetail.ets` | HAP+VM(Phone) |
...
| **Full Directory** | `23 · Pages · Final` `943:9888`/`943:10196` + `15 · Reader 2` `1023:18662`/`1023:18664` | Phone/Tablet | CORE | `local_book.toc` | 详情→目录；选章→该章正文；返回=详情 | ✅ | ⚠️（单次选章链路接通 + `chapterSelectionToken` 防并发覆盖；但 `R-LOCAL-TOC-01` 仍存：Phone/Tablet 实例无 prototype reaction，章节行固定样例，无本地目录字段→正文映射） | ✅（Phone） | ⊘Core阻塞（视觉合同未闭合） | `R-LOCAL-TOC-01` | `ReaderFullDirectory.ets` + `FullDirectoryPanel.ets` | HAP+VM(Phone，无书未验证) |
...
- **VM 时间线**：VM 中运行的是 10:48 安装的候选版本；第四轮 HAP 生成于 11:36 后，未重新安装，设备侧无法 SHA 比对，**不能称"当前构建已完成安装回归"**。
