...
APP = int(next(json.loads(l)['appPid'] for f in ROOT.glob('*-commands.jsonl') for l in f.read_text().splitlines() if 'appPid' in json.loads(l)))
...
def analyze(path):
        ...
        if kind == 'sched_switch':
            ...
            if fields[0] == 'B' and len(fields) >= 3:
                ...
                if stacks[tid]:
                    ...
                    if tid == APP or 'ReaderBookTurn.' in name:
                        slices.append(dict(tid=tid, start=begin, end=ts, wallMs=(ts-begin)*1000, name=name))
                else:
                    unmatched[tid] += 1
    for s in slices:
        s['guestScheduledMs'] = sum(max(0, min(s['end'], b)-max(s['start'], a)) for a,b in intervals[s['tid']])*1000
    ...
    return dict(file=path.name, entries=re.findall(r'entries-in-buffer/entries-written:.*', raw)[:1],
                ...
                frames=frames, inputs=inputs, longestCustomUpdates=custom[:30], nativeUploadSlices=[s for s in slices if 'ReaderBookTurn.' in s['name']],
...
for path in sorted(ROOT.glob('*trace.ftrace')):
    if path.name.startswith(('slide-quiet','quiet-tool')): continue
    r=analyze(path)
    results.append(r)
    f=r['frames']
