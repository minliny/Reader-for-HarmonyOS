...
run(['shell','uitest','uiInput','swipe','800','1800','800','1300','600']);time.sleep(1)
before=capture('before-exit')
...
after=capture('after-reopen')
def rect(a): return list(map(int,re.findall(r'-?\d+',a['origBounds'])))
assert len(before)==len(after)==1 and before[0]['text']==after[0]['text']
...
(out/(label+'-result.json')).write_text(json.dumps(result,ensure_ascii=False,indent=2));print(json.dumps(result,ensure_ascii=False),flush=True)
