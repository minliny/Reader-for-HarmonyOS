...
choices=visibleText(tree,mode);assert len(choices)==1
assert choices[0].get('enabled')=='true'
click(*center(choices[0]));tree=dump();chosen=visibleText(tree,mode)
assert len(chosen)==1 and chosen[0].get('backgroundColor') not in [None,'#00000000']
...
x,y=center(g[0]);run(['shell','uitest','uiInput','swipe',str(x),str(y),str(x),'2720','1200']);time.sleep(.7)
tree=dump();assert not visibleText(tree,'设置')
(out/(label+'-result.json')).write_text(json.dumps(dict(mode=mode,status='selected-confirmed-and-controls-closed',scope='mode selection only'),ensure_ascii=False,indent=2))
print(json.dumps(dict(status='selected-confirmed',label=label)),flush=True)
