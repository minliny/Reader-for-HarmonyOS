...
g=[a for a,o,_ in nodes(tree) if o>.99 and a.get('id')=='reader-control-motion-grabber'];assert len(g)==1;x,y=center(g[0]);run(['shell','uitest','uiInput','swipe',str(x),str(y),str(x),'2720','1200']);time.sleep(.7);tree=dump();assert not visibleText(tree,'设置');origin=page(tree);results=[]
for turn in range(rounds*2):
 nextward=turn%2==0;x1,x2=(1000,280) if nextward else (280,1000)
 run(['shell','uitest','uiInput','swipe',str(x1),'1350',str(x2),'1350','900']);time.sleep(.45);tree=dump();actual=page(tree)
 assert (actual['sha256']!=origin['sha256']) if nextward else (actual['sha256']==origin['sha256']),f'roundtrip mismatch at turn {turn+1}: {actual}'
 results.append(dict(turn=turn+1,direction='next' if nextward else 'previous',page=actual))
 if (turn+1)%10==0:print(json.dumps(dict(label=label,turns=turn+1,status='content-roundtrip-pass')),flush=True)
(out/(label+'-result.json')).write_text(json.dumps(dict(mode=mode,origin=origin,results=results,scope='settled-content roundtrips only; not latency, display frames, quiet hold or rapid throughput'),ensure_ascii=False,indent=2))
print(json.dumps(dict(status='content-roundtrip-pass',label=label,turns=len(results))),flush=True)
