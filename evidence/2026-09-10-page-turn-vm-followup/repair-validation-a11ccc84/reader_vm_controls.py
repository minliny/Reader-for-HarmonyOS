...
appPid=int(run(['shell','pidof','io.reader.harmonyos']).strip())
with log.open('a') as f:f.write(json.dumps({'appPid':appPid})+'\n')
print('app pid:',appPid,flush=True)
...
(out/(label+'-result.json')).write_text(json.dumps(dict(status='PASS',module=module,scope='open, drag expand, automatic expand, automatic collapse, drag close; endpoint geometry only; trace performance separate'),ensure_ascii=False,indent=2));print(module,'PASS',flush=True)
