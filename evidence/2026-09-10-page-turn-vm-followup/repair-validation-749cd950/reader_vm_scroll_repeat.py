...
def capture(suffix):
 ...
 tree=dump()
 rows=[a for a,o,_ in nodes(tree) if o>.99 and len(a.get('text',''))>200]
 assert rows,'continuous text rows required'
 return [{k:a.get(k) for k in ['text','bounds','origBounds']} for a in rows]
initial=capture('initial')
for iteration in range(rounds):
 before=capture('before-exit-'+str(iteration))
 run(['shell','uitest','uiInput','keyEvent','Back']);time.sleep(.5);tree=dump()
 assert visibleText(tree,'书架') and any('ReaderIndentCRLF' in a.get('text','') for a,o,_ in nodes(tree) if o>.99)
 reading=visibleText(tree,'阅读');assert len(reading)==1;click(*center(reading[0]));time.sleep(1)
 after=capture('after-reopen-'+str(iteration))
 def rect(a): return list(map(int,re.findall(r'-?\d+',a['origBounds'])))
 assert before and len(before)==len(after) and [r['text'] for r in before]==[r['text'] for r in after], 'visible content changed'
 b,a=rect(before[0]),rect(after[0]);delta=a[1]-b[1]
 result=dict(status='PASS' if abs(delta)<=8 else 'FAIL',verticalRestoreDeltaPx=delta,before=before[0]['origBounds'],after=after[0]['origBounds'],scope='one short scroll and production exit/reopen, not failures/images/cross-chapter',book='ReaderIndentCRLF20260910')
 (out/(label+'-'+str(iteration)+'-result.json')).write_text(json.dumps(result,ensure_ascii=False,indent=2));print(json.dumps(result,ensure_ascii=False),flush=True)
