...
> 历史审计基线：下文“当前/尚未实施”指 PH71 审计时点。后续已实现正文与位置原子提交、mark 归属、处理版本和清缓存保护；最新实现、保守恢复边界及分层门禁以 [实施回执](search-flow-implementation/IMPLEMENTATION.md) 和 [READER_REPAIR_SPEC §16.11](../../docs/READER_REPAIR_SPEC.md) 为准。本页保留失败探针，不作为当前缺口状态。
