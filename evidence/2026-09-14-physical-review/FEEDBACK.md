...
- **该触发的代码补修**：已复现显示回执后的零区域清空测量、底色 Row 未订阅后续测量两个缺口。已增加同几何的显示过渡保留及 Row 实际观测依赖，八主题/开关/快速显隐/窗口替换专项通过；不修改主题色。详见[触发专项](theme-status-ph98/toggle-off/REPORT.md)。本地结论不等于用户现场唯一根因或设备像素通过，尚未安装。
