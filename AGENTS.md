...
11. `candidate-backport` is a STOP condition, not a renderable state. A page family whose `harmony.status` is `candidate-backport` (or whose generated admission entry says `candidate-backport`) has NOT completed Reader-UI source-side conversion and has NOT been consumed by HarmonyOS. It must NOT enter a virtual-machine or device test cycle. RouteRenderer, ViewStateRenderer, and OverlayHost consume only the generated Reader-UI admission; an active Reader-UI route-reconstruction quarantine additionally removes old RouteTable/ViewStateTable mappings. The generic `StateHost` is retired rather than hidden. Do not bypass either guard by editing a renderer or re-adding a local route map.
